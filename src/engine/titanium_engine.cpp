...
void TitaniumEngine::process_orders_batched(const Order* orders,
    ...
    std::thread gpu_worker([&]() {
        ...
        while (offset < total_count) {
            ...
            risk_engine.synchronize(); // Synchronize the slot loop
...
void TitaniumEngine::match_buy(Order& order) {
    ...
    while (order.quantity > 0) {
        ...
        while (level->order_count > 0 && order.quantity > 0) {
            ...
            pending_trades_.push_back({order.id, resting.id, resting.price, traded});
...
void TitaniumEngine::match_sell(Order& order) {
    ...
    while (order.quantity > 0) {
        ...
        while (level->order_count > 0 && order.quantity > 0) {
            ...
            pending_trades_.push_back({resting.id, order.id, resting.price, traded});
